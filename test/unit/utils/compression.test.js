...
describe('utils/compression.js', () => {
  ...
  it('should NOT double compress and decompress', () => {
    const event = JSON.stringify(eventObject);
    const compressed = JSON.stringify(eventObject, compress({ compressionThreshold: 512 }));
    // change prevents this mistake
    const doubleCompressed = JSON.stringify(JSON.parse(compressed), compress({ compressionThreshold: 512 }));
    const decompressed = JSON.parse(doubleCompressed, decompress);
